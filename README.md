...
删除 `[全UP蓝点] 复活警告` 控制台输出。它只是排查期的临时诊断手段，真正的根因
（直播场次状态机 + 72 小时时效护栏 + 超窗自愈）已在 v1.5.4~v1.5.7 修复，继续保留
反而会误导判断。

### v1.5.7 — 直播提醒默认关闭

B 站的直播在动态流/空间页没有可点开看的「直播动态」卡片，蓝点点开后无内容可看，
体感等于误报。改为默认关闭，存量配置自动迁移；关闭后原有的直播未读会自动清除。
想要开播提醒可在弹窗里勾选「直播开播」，勾选后不再被自动关闭。

### v1.5.4 ~ v1.5.6 — 直播场次状态机 / 时效护栏 / 自愈

- **直播场次状态机**：旧逻辑每轮扫描对直播中的 UP 无条件重新点亮，导致蓝点点掉后
  每 30 秒必复活。改为只在新场次（下播→开播跳变 / 场次键变化）点亮一次，并附带
  10 分钟抖动保护。
- **72 小时时效护栏**：补扫翻到的超窗旧动态（如刚关注 UP 的历史动态）只建档不点亮。
- **超窗自愈**：历史错误点亮的残留未读，下一轮扫描自动熄灭。
- 修复 `cleanup()` 中的 `st` 引用错误（每轮扫描抛异常，导致落盘与 SCAN_DONE 中断）。
